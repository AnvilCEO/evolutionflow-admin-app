...
import { ScheduleContent, getAdminSchedule } from "@/lib/api/admin";
...
export default function ScheduleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchSchedule() {
      ...
      try {
        setIsLoading(true);
        const response = await getAdminSchedule(id, accessToken);
        setFormData(response.data);
        if (response.data.imageUrl) {
          // For mock, we just set a dummy URL if one exists
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setFormData((prev) => ({ ...prev, imageUrl: "updated-url.png" }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mock API call to save edits
      await new Promise(resolve => setTimeout(resolve, 800));

      console.log("Updating schedule:", id, formData);

      router.push("/admin/schedules");
      router.refresh();
    } catch (err: unknown) {
      console.error("Failed to update schedule:", err);
      setError("스케줄을 수정하는 중 오류가 발생했습니다.");
    } finally {
  ...
  const handleDelete = async () => {
    if (!confirm("정말 이 스케줄을 삭제하시겠습니까? 관련 예약 내역에 영향을 줄 수 있습니다.")) {
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <LoadingSpinner message="스케줄 정보를 불러오는 중..." />
      </div>
    );
  }
    ...
    <div className="max-w-4xl max-h-[calc(100vh-100px)] overflow-y-auto no-scrollbar space-y-6 pb-20">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <h1 className="font-pretendard text-3xl font-bold text-black border-l-4 border-black pl-3">
            스케줄 정보 수정
          </h1>
          <p className="mt-2 text-sm text-gray-600">
            등록된 달력 스케줄을 확인하고 수정합니다.
          </p>
        </div>
        <button
          onClick={handleDelete}
          className="rounded-md border border-red-200 bg-red-50 text-red-600 px-4 py-2 text-sm font-medium hover:bg-red-100 transition-colors"
        >
          스케줄 영구 삭제
        </button>
      </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-black focus:outline-none focus:ring-1 focus:ring-black transition-colors bg-white font-medium text-gray-900"
              >
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    이미지를 변경하려면 새 파일을 업로드하세요. 권장 비율 1:1.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-lg border border-gray-100 shadow-sm">
          ...
          <div className="mt-8 flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">이 스케줄을 사용자 목록에 노출할지 결정합니다.</p>
            </div>
